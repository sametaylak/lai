...
void _darray_pop(void *array, void *dest) {
  ...
  lai_copy_memory(dest, (void *)addr, stride);
  _darray_field_set(array, DARRAY_LENGTH, length - 1);
}
...
void *_darray_insert_at(void *array, u64 index, const void *value_ptr) {
  ...
  if (index != length - 1) {
    lai_copy_memory((void *)(addr + ((index + 1) * stride)),
                    (void *)(addr + (index * stride)),
                    stride * (length - index));
  }

  lai_copy_memory((void *)(addr + (index * stride)), value_ptr, stride);
...
void *_darray_pop_at(void *array, u64 index, void *dest) {
  ...
  u64 addr = (u64)array;
  lai_copy_memory(dest, (void *)(addr + (index * stride)), stride);

  if (index != length - 1) {
    lai_copy_memory((void *)(addr + (index * stride)),
